...
output_dir = "anomalyDetection/data/average_activity_type_clicks_avg"
file_name = "all_students_average_activity_type_clicks"
...
def get_raw_data():
    ...
    vle_list = pd.read_sql_query("""
        SELECT SV.id_student, SV.id_site, avg(SV.sum_click) as sum_click FROM StudentVle SV,Vle V 
        WHERE  SV.id_site=V.id_site and V.code_module = %(code_module)s 
        GROUP BY SV.id_student, SV.id_site 
        ORDER BY SV.id_student, SV.id_site
        """,
        database_connection,
        params={"code_module": code_module},
    )
...
def stratified_k_fold(df: pd.DataFrame):
